...
def k_means(num_cluster, x_point, y_point, points_number):
    ...
    while is_last_step:
        if is_first_step:
            is_first_step = False
            start_point = np.random.choice(range(points_number), num_cluster, replace=False)#ставятся центроиды
            centroid = x_point[start_point]
        else:
            prev_centroid = np.copy(centroid)
            for i in range(num_cluster):
                centroid[i] = np.mean(x_point[y_point == i], axis=0) #среднее значение расстояния до центроида
                                                                     #(изменение координаты центроида)
